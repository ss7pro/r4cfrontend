...
{
  $jq_path = sfConfig::get('sf_jquery_web_dir', '/sfJqueryReloadedPlugin') . '/js/' . sfConfig::get('sf_jquery_core', 'jquery-1.5.1.min.js');
}
sfContext::getInstance()->getResponse()->addJavascript($jq_path, 'first');

$jq_ui     = sfConfig::get('sf_jquery_web_dir', '/sfJqueryReloadedPlugin') . '/js/plugins/'  . sfConfig::get('jquery_ui', 'jquery-ui-1.8.11.custom.min.js');
$jq_ui_css = sfConfig::get('sf_jquery_web_dir', '/sfJqueryReloadedPlugin') . '/css/' . sfConfig::get('sf_jquery_ui_css_style', 'ui-lightness') . '/' . sfConfig::get('sf_jquery_ui_css', 'jquery-ui-1.8.11.custom.css');
sfContext::getInstance()->getResponse()->addJavascript($jq_ui, 'first');
sfContext::getInstance()->getResponse()->addStylesheet($jq_ui_css, 'first');
...
function jq_add_plugins_by_name($args = array()) {
  ...
  $plugins = array(
    ...
    'sortable' => sfConfig::get('sf_jquery_ui', 'jquery-ui-1.7.3.custom.min.js'),
    'ui' => sfConfig::get('sf_jquery_ui', 'jquery-ui-1.7.3.custom.min.js'),
    'autocomplete' => 'jquery.autocomplete.min.js'
  );
  $plugins = array_merge($plugins, (array)sfConfig::get('sf_jquery_plugins', array()));
...
function jq_add_plugins_css($args = array())
{
  $plugins = sfConfig::get('sf_jquery_plugins_css', array());
  $pluginPaths = sfConfig::get('sf_jquery_plugin_paths_css');
  foreach ($args as $name)
  {
    if (!isset($plugins[$name]))
    {
      throw new Exception("Unknown jQuery plugin css $name");
    }
    if (!isset($pluginPaths[$name]))
    {
      $filename = sfConfig::get("sf_jquery_{$name}_css", $plugins[$name]);
      $filename = sfConfig::get('sf_jquery_web_dir', '/sfJqueryReloadedPlugin') . "/css/$filename";
    } else {
      $filename = $pluginPaths[$name];
    }
